...
def load_points_from_file(filename, delimiter=',', dimension = 3, dtype=tf.float32):
  ...
  Args:
    ...
    dimension: `int` D1 , the first D1 elements in each line are treated as point coordinates, the rest as features
...
def load_batch_of_points(filenames, batch_shape=[-1], delimiter=',', point_dimension=3, dtype=tf.float32):
  """ Loads a batch of point clouds form the given ASCII files and creates zero padded tensor
  for the point coordinates and the features and a 'sizes' tensor with the number of points per point cloud.
...
def load_batch_of_meshes(filenames, batch_shape=[-1], file_type=None, dtype=tf.float32, **kwargs):
  """ Loads a batch of point clouds from the given mesh files and creates a zero padded tensor
  for the point coordinates and a 'sizes' tensor with the number of points per point cloud.
  ...
  for i in range(batch_size):
    pad_size = max_num_points - sizes[i]
    points[i] = tf.concat((points[i], tf.zeros(shape=[pad_size,3], dtype=dtype)),axis=0)
  points = tf.stack(values=points,axis=0)
