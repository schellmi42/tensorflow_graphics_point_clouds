...
def compute_keys(pPointCloud, pAABB, pNumCells, pCellSize, name=None):
    with tf.compat.v1.name_scope(name, "compute keys", [pPointCloud, pAABB, pNumCells, pCellSize]):
...
def build_grid_ds(pKeys, pNumCells, pbatch_size, name=None):
    with tf.compat.v1.name_scope(name, "build grid ds", [pKeys, pNumCells, pbatch_size]):
...
def find_neighbors(pGrid, pPCSamples, pRadii, pMaxNeighbors, name=None): 
    with tf.compat.v1.name_scope(name, "find neighbours",[pGrid, pPCSamples, pRadii, pMaxNeighbors]):  
